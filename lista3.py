...
# --- ABC ---
N_ABC = 100_000        # nº total de simulações
FRAC_ACEITE = 0.01     # fração aceita (top 1%)
...
def simular_fila(lam, mu, tempo_max=TEMPO_MAX, n_serv=N_SERV):
    ...
    """
    # Gerar interchegadas ~ Exp(λ). Geramos mais do que o esperado
    # para garantir cobertura de todo o intervalo [0, tempo_max].
...
def executar_abc(n_amostras, lam_range, mu_range, frac_aceite, rotulo=""):
    """
    Executa ABC rejection sampling para os parâmetros (λ, μ) de uma fila M/M/2.

    Gera n_amostras candidatos das prioris uniformes nos intervalos dados,
    simula uma fila para cada, calcula as estatísticas resumo (w̄, Lq),
    e aceita os top 'frac_aceite' por menor distância ao observado.

    Parâmetros:
        n_amostras  : número total de simulações
        lam_range   : (min, max) da priori de λ
        mu_range    : (min, max) da priori de μ
        frac_aceite : fração aceita (ex: 0.01 = top 1%)
        rotulo      : nome do estágio para log

    Retorna:
        dict com arrays das amostras aceitas e diagnósticos
    """
    lam_min_r, lam_max_r = lam_range
    mu_min_r, mu_max_r = mu_range

    lam_cands = np.random.uniform(lam_min_r, lam_max_r, size=n_amostras)
    mu_cands = np.random.uniform(mu_min_r, mu_max_r, size=n_amostras)

    w_arr = np.zeros(n_amostras)
    lq_arr = np.zeros(n_amostras)
    dist_arr = np.full(n_amostras, np.inf)

    t0 = time.time()
    for i in range(n_amostras):
        if (i + 1) % 20000 == 0:
            elapsed = time.time() - t0
            pct = 100 * (i + 1) / n_amostras
            print(f"    [{pct:5.1f}%] {i+1:,}/{n_amostras:,} ({elapsed:.1f}s)")

        res = simular_fila(lam_cands[i], mu_cands[i])
        if res['n_clientes'] < 3:
            continue

        w_i = np.mean(res['esperas'])
        lq_i = calcular_lq_medio(
            res['chegadas'], res['partidas'], TEMPO_MAX, N_SERV
        )

        w_arr[i] = w_i
        lq_arr[i] = lq_i
        dist_arr[i] = calcular_distancia(w_i, lq_i)

    # Selecionar top frac_aceite
    n_aceitos = max(int(n_amostras * frac_aceite), 50)
    idx_sort = np.argsort(dist_arr)
    idx_ok = idx_sort[:n_aceitos]
    eps = dist_arr[idx_ok[-1]]
    ...
    return {
        'lam': lam_cands[idx_ok], 'mu': mu_cands[idx_ok],
        'w': w_arr[idx_ok], 'lq': lq_arr[idx_ok],
        'dist': dist_arr[idx_ok], 'eps': eps,
        'n_aceitos': n_aceitos, 'tempo': tempo_total
    }


# =====================================================================
# QUESTÃO 1 — ESTIMAÇÃO DA POSTERIORI VIA ABC (2 ESTÁGIOS)
# =====================================================================
...
print(f"Estratégia: ABC em 2 estágios (ajuste recursivo das prioris)\n")

# ---- ESTÁGIO 1: prioris amplas, aceitação generosa ----
# Objetivo: localizar a região do espaço paramétrico relevante
print(f"  Estágio 1: prioris amplas, 5% de aceitação")
print(f"    λ ~ U({LAM_MIN}, {LAM_MAX}), μ ~ U({MU_MIN}, {MU_MAX})")
...
# ---- Definir prioris refinadas para o Estágio 2 ----
# Usa o range das amostras aceitas no Estágio 1, com margem de segurança,
# para concentrar a amostragem na região de interesse.
MARGEM = 0.20  # 20% de folga
lam_e2_min = max(LAM_MIN, np.quantile(res_e1['lam'], 0.02) * (1 - MARGEM))
lam_e2_max = min(LAM_MAX, np.quantile(res_e1['lam'], 0.98) * (1 + MARGEM))
mu_e2_min = max(MU_MIN, np.quantile(res_e1['mu'], 0.02) * (1 - MARGEM))
mu_e2_max = min(MU_MAX, np.quantile(res_e1['mu'], 0.98) * (1 + MARGEM))

print(f"\n  Prioris refinadas para Estágio 2:")
print(f"    λ ~ U({lam_e2_min:.4f}, {lam_e2_max:.4f})")
print(f"    μ ~ U({mu_e2_min:.4f}, {mu_e2_max:.4f})")

# ---- ESTÁGIO 2: prioris refinadas, aceitação mais rigorosa ----
print(f"\n  Estágio 2: prioris refinadas, {FRAC_ACEITE*100:.1f}% de aceitação")

res_e2 = executar_abc(
    n_amostras=N_ABC,
    lam_range=(lam_e2_min, lam_e2_max),
    mu_range=(mu_e2_min, mu_e2_max),
    frac_aceite=FRAC_ACEITE,
    rotulo="Estágio 2"
)
...
w_post = res_e2['w']
lq_post = res_e2['lq']
dist_post = res_e2['dist']
eps_final = res_e2['eps']
rho_post = lam_post / (2 * mu_post)

t1_q1 = time.time()

print(f"\n--- Resultados Finais do ABC (Estágio 2) ---")
print(f"  Amostras aceitas: {res_e2['n_aceitos']}")
print(f"  ε (distância máxima aceita): {eps_final:.4f}")
...
plt.suptitle('Questão 1 — ABC Rejection Sampling (2 estágios)',
...
for i in range(n_post):
    ...
    cadeiras[i] = calcular_quantil_lq(
        res_q3['chegadas'], res_q3['partidas'], TEMPO_MAX, N_SERV, 0.90
    )
...
for i in range(n_post):
    # --- Cenário A: fila única M/M/2 ---
    res_a = simular_fila(lam_post[i], mu_post[i], n_serv=2)
    w_mm2_arr[i] = np.mean(res_a['esperas']) if res_a['n_clientes'] > 0 else 0

    # --- Cenário B: duas filas M/M/1 independentes (λ/2 cada) ---
    res_b1 = simular_fila(lam_post[i] / 2, mu_post[i], n_serv=1)
    res_b2 = simular_fila(lam_post[i] / 2, mu_post[i], n_serv=1)

    total_b = res_b1['n_clientes'] + res_b2['n_clientes']
    if total_b > 0:
        soma_esp = np.sum(res_b1['esperas']) + np.sum(res_b2['esperas'])
        w_2mm1_arr[i] = soma_esp / total_b
    else:
        w_2mm1_arr[i] = 0
...
# Cenários com diferentes perfis de tolerância
cenarios = [
    {'nome': 'Impaciente',    'alpha': 0.30, 'k0': 10},
    {'nome': 'Intermediário', 'alpha': 0.25, 'k0': 15},
    {'nome': 'Tolerante',     'alpha': 0.20, 'k0': 20},
]

# --- Gráfico da função de desistência ---
plt.figure(figsize=(10, 5))
k_vals = np.arange(0, 61)
for cen in cenarios:
    p_vals = prob_desistencia(k_vals, cen['alpha'], cen['k0'])
    plt.plot(k_vals, p_vals, lw=2.5,
             label=f"{cen['nome']} (α={cen['alpha']}, k₀={cen['k0']})")
plt.xlabel('Lq (pessoas na fila de espera)')
plt.ylabel('P(desistir)')
plt.title('Questão 5 — Função de Desistência (Logística)')
plt.legend()
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig('q5_funcao_balking.png', dpi=300, bbox_inches='tight')
print("Gráfico salvo: q5_funcao_balking.png")
plt.close()

# --- Simular cada cenário para todas as amostras da posteriori ---
resultados_q5 = {}

for cen in cenarios:
    desist_arr = np.zeros(n_post, dtype=int)
    potenc_arr = np.zeros(n_post, dtype=int)

    print(f"\n  Cenário: {cen['nome']} "
          f"(α={cen['alpha']}, k₀={cen['k0']})")

    for i in range(n_post):
        res_balk = simular_fila_balking(
            lam_post[i], mu_post[i], TEMPO_MAX, N_SERV,
            cen['alpha'], cen['k0']
        )
        desist_arr[i] = res_balk['desistencias']
        potenc_arr[i] = res_balk['potenciais']

    resultados_q5[cen['nome']] = {
        'desistencias': desist_arr,
        'potenciais': potenc_arr
    }

    # Evitar divisão por zero
    taxas = np.where(potenc_arr > 0,
                     desist_arr / potenc_arr, 0)
    ...
    print(f"    Taxa de desistência: {100*np.mean(taxas):.1f}%")

t1_q5 = time.time()
print(f"\n  Tempo Q5: {t1_q5 - t0_q5:.1f}s")

# --- Gráfico Q5: histogramas de desistências ---
fig, axes = plt.subplots(1, 3, figsize=(16, 5))
cores = ['#e74c3c', '#f39c12', '#27ae60']

for idx, cen in enumerate(cenarios):
    ax = axes[idx]
    dados = resultados_q5[cen['nome']]['desistencias']

    ax.hist(dados, bins=30, density=True, alpha=0.7,
            color=cores[idx], edgecolor='white')
    ax.axvline(np.median(dados), color='black', ls='--', lw=2,
               label=f'Mediana = {np.median(dados):.0f}')
    ax.set_xlabel('Nº de Desistências')
    ax.set_ylabel('Densidade')
    ax.set_title(f"{cen['nome']}\n(α={cen['alpha']}, k₀={cen['k0']})")
    ax.legend(fontsize=9)
    ax.grid(True, alpha=0.3)

plt.suptitle('Questão 5 — Estimativa de Clientes que Desistiram',
             fontsize=13, fontweight='bold')
plt.tight_layout()
plt.savefig('q5_desistencias.png', dpi=300, bbox_inches='tight')
print("Gráfico salvo: q5_desistencias.png")
plt.close()
...
for cen in cenarios:
    d = resultados_q5[cen['nome']]['desistencias']
    print(f"  Q5 ({cen['nome']:13s}): "
          f"{np.median(d):.0f} desistências (mediana)")
